...
async function getData(number) {
    let query = {};
    const moment = require('moment-timezone');
    const specificDate = moment.tz('2023-12-08', 'Asia/Ho_Chi_Minh');
    let startDateTime = null;
    let endDateTime = null;
    if (number == 1) {
        startDateTime = specificDate.clone().startOf('day').hour(7);
        endDateTime = specificDate.clone().startOf('day').hour(22);
    }
    else {
    ...
    query.$and.push(
        {
            $or: [
                { DateTimeIn: { $gte: startDateTime.toDate(), $lte: endDateTime.toDate() } },
                {
                    $and: [
                        { DateTimeOut: { $ne: null } },
                        { DateTimeOut: { $gte: startDateTime.toDate(), $lte: endDateTime.toDate() } },
                    ],
                },
            ],
        }
